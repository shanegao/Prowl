...
| **Advanced** | Analytics, crash reports, restore terminal layout on launch (experimental) + clear saved layout, the **Quick Send skills directory** (folder whose subfolders become `/skill` completions in the quick-send composer; empty = the selected agent's global `~/.<agent>/skills` plus its worktree's project-local `./.<agent>/skills`), and the **Install Command Line Tool** (`prowl` CLI) action. |
