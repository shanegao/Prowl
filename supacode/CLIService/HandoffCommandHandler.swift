...
final class HandoffCommandHandler: CommandHandler {
  ...
  /// Atomically claims a HUD request before its transition starts.
  typealias RequestAuthorizer = (UUID) -> Bool
  ...
  func handle(envelope: CommandEnvelope, context: CLICommandContext) async -> CommandResponse {
    ...
    if let requestID = input.requestID, !requestAuthorizer(requestID) {
      return errorResponse(
